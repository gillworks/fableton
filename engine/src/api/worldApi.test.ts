...
// the sim spawns it into construction mid-run (issue #107). Its render def must
// still reach the client: the plan's queue entries carry the full
// construction_site, so /api/construction pairs the live state with position,
// stage meshes, and completion payload — proving the served-world wiring, not
// just the engine, closes the loop end to end.
describe('world-api construction from an expansion plan (issue #107)', () => {
  ...
  beforeAll(async () => {
    // Note: no `sites` — the site reaches construction ONLY via the plan, and
    // its render def reaches the API ONLY via deps.expansionPlan.
