...
First boot founds the world into the `world-data` volume: the default charter boots the flagship's hand-authored starter world (residents included); point `FABLETON_CHARTER` at any other charter in the image to generate a fresh skeleton world from it. Every boot re-runs the validation gate before serving — an invalid world never comes up. `docker compose down -v` forgets the world; without `-v` it persists.
